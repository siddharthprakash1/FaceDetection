...
while True:
    ...
    for encoding in encodings:
        ...
        # otherwise, we get the name with the highest number of matches
            ...
            name = max(counts, key=counts.get)
            # compute the confidence level as a percentage of the highest number of matches
            confidence = (counts[name] / sum(counts.values())) * 100

        # add the name and confidence level to the list of names
        names.append((name, confidence))

    # loop over the `rectangles` of the faces in the
    # input frame using the `face_rects` function
    for rect, (name, confidence) in zip(face_rects(frame), names):
        # get the bounding box for each face using the `rect` variable
        x1, y1, x2, y2 = rect.left(), rect.top(), rect.right(), rect.bottom()
        # draw the bounding box of the face along with the name and confidence level
        cv2.rectangle(frame, (x1, y1), (x2, y2), (0, 255, 0), 2)
